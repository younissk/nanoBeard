...
def generate(
    ...
    block_size: int = cast(Config, model.config).block_size
...
def chat_repl(model: nn.Module, tokenizer: Tokenizer, args: argparse.Namespace) -> None:
    ...
    if eos_id is None:
        raise ValueError("tokenizer has no <|endoftext|> token; chat needs one to stop")
