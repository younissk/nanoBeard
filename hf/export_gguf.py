...
Which Python runs the converter:
    llama.cpp's ``convert_hf_to_gguf.py`` needs ``gguf``,
    ``transformers==5.5.1`` (the version llama.cpp targets) and
    ``sentencepiece`` (Qwen3's ``set_vocab`` imports it before falling back to
    the BPE path). Those cannot live in nanobeard's env: ``transformers==5.5.1``
    pins ``tokenizers<=0.23.0`` while nanobeard needs ``tokenizers>=0.23.1``, so
    uv rejects the combination outright.

    So the converter gets its own interpreter. Resolution order:
      1. ``--converter-python`` if passed,
      2. ``$NANOBEARD_CONVERTER_PYTHON`` if set,
      3. a venv inside the llama.cpp checkout (``.venv`` or ``venv``),
      4. ``sys.executable`` — the old behaviour, kept as a fallback.
    The chosen interpreter is checked for the three imports before conversion
    starts, so a missing dep fails in a second with the exact fix to run rather
    than deep inside the converter.

    One-time setup (from the llama.cpp checkout):
        uv venv .venv
        .venv/bin/python -m pip install gguf transformers==5.5.1 sentencepiece

Usage:
    uv run python -m scripts.export.export_gguf \
...
# --------------------------------------------------------------------------
def remap_to_qwen3(
    state: dict[str, torch.Tensor], n_layer: int, n_embd: int
...
CONVERTER_DEPS = ("gguf", "transformers", "sentencepiece")


def resolve_converter_python(explicit: str | None, llama_cpp: Path) -> str:
    """Pick the interpreter that runs convert_hf_to_gguf.py.

    See the module docstring for why this is not simply ``sys.executable``.
    """
    if explicit:
        return explicit
    env = os.environ.get("NANOBEARD_CONVERTER_PYTHON")
    if env:
        return env
    for venv in (".venv", "venv"):
        cand = llama_cpp / venv / "bin" / "python"
        if cand.exists():
            return str(cand)
    return sys.executable


def check_converter_deps(python: str) -> None:
    """Fail fast, with the exact install command, if the converter env is bare."""
    probe = "import " + ", ".join(CONVERTER_DEPS)
    proc = subprocess.run([python, "-c", probe], capture_output=True, text=True)
    if proc.returncode == 0:
        return
    missing = (proc.stderr.strip().splitlines() or ["<unknown import error>"])[-1]
    same_env = Path(python).resolve() == Path(sys.executable).resolve()
    hint = (
        "That is nanobeard's own env, which cannot hold transformers==5.5.1 "
        "(it pins tokenizers<=0.23.0). Give the converter its own interpreter:\n"
        "    uv venv <llama.cpp>/.venv\n"
        "    <llama.cpp>/.venv/bin/python -m pip install "
        + " ".join(("gguf", "transformers==5.5.1", "sentencepiece"))
        + "\nthen re-run, or pass --converter-python explicitly."
        if same_env
        else f"    {python} -m pip install gguf transformers==5.5.1 sentencepiece"
    )
    sys.exit(f"converter env is missing a dependency\n  {python}\n  {missing}\n{hint}")


def convert_to_f16(
    hf_dir: Path, out_gguf: Path, llama_cpp: Path, python: str | None = None
) -> None:
...
def quantize(f16_gguf: Path, out_gguf: Path, quant: str) -> None:
    ...
    proc = subprocess.run(
        [binary, str(f16_gguf), str(out_gguf), quant], capture_output=True, text=True
    )
    if proc.returncode != 0:
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Interpreter that runs llama.cpp's convert_hf_to_gguf.py. Defaults to "
            "$NANOBEARD_CONVERTER_PYTHON, then a venv in the llama.cpp checkout, "
            "then this interpreter. Keeps transformers==5.5.1 out of nanobeard's env."
        ),
    )
    ...
    print(
        f"      arch: L{meta['n_layer']} d{meta['n_embd']} h{meta['n_head']} "
    ...
    converter_python = resolve_converter_python(args.converter_python, llama_cpp)
    check_converter_deps(converter_python)
    print(f"[2/4] convert -> f16 GGUF  (converter python: {converter_python})")
    convert_to_f16(hf_dir, f16, llama_cpp, converter_python)
